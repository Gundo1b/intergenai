...
export function LanguageProvider({ children }: { children: React.ReactNode }) {
  ...
  const t = (key: string): string => {
    ...
    // Fallback to English if selected language has no translation bundle
    const langBundle: any = (translations as any)[language] || translations['en'];
    let value: any = langBundle;

    for (const k of keys) {
      value = value?.[k];
    }

    // Specific copy refinement: ensure privacy label reads "Privacy Policy"
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-8 md:gap-12">
            <div>
              ...
              <ul className="space-y-2 text-sm md:text-base">
                ...
                <li>
                  <Link to="/ai-code" className="text-slate-600 hover:text-brand-end dark:text-gray-400 dark:hover:text-white transition-colors">{t('footer.aiCode')}</Link>
                </li>
                <li>
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm md:text-base">
                ...
                <li>
                  <Link to="/terms" className="text-slate-600 hover:text-brand-end dark:text-gray-400 dark:hover:text-white transition-colors">{t('footer.terms')}</Link>
                </li>
              </ul>
            </div>
