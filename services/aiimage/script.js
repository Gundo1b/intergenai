...
function setResultView(view) {
  ...
  previewImage.hidden = !isPreview;
  jsonOutput.hidden = isPreview;

  // If nothing generated yet, keep placeholder visible
  if (!previewImage.src && jsonOutput.textContent.trim() === "") {
...
async function runGeneration({ prompt }) {
  ...
  try {
    ...
    // Optional: placeholder image (gradient) so preview isn’t empty
      ...
      btoa(`
        <svg xmlns="http://www.w3.org/2000/svg" width="1024" height="1024">
          ...
          <text x="60" y="230" font-size="28" fill="rgba(255,255,255,0.80)" font-family="Arial">
            Replace this with your generated image output
          </text>
        </svg>
      `);
